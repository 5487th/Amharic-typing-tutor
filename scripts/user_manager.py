...
class UserManager:
    def __init__(self):
        ...
        self.default_profile_picture_path = (
        ...
        self.user_Data_base_location = (
            pathlib.Path(__file__).parent.parent
            / "data"
            / f"{self.usermanager_database_name}.db"
        )
    ...
    def create_user(self, username, password="", profile_picture_path="") -> bool:
        ...
        self.usermanager_database_cursor.execute(
            f"SELECT (username) FROM {self.users_table_name} Where username = ?",
            (username,),
        )
        ...
        if not username:
            warnings.warn(
                'called create user function with a "None" username, user not created'
            )
            return False

        if user_already_exists:
            warnings.warn(
                "called create user with an existing username, new user not created"
            )
            return False

        if password == None:
            warnings.warn(
                'called create user function with a "None" password, user not created'
            )
            return False
    ...
    def delete_user(self, username_of_user_to_be_deleted):
        if username_of_user_to_be_deleted:
            self.usermanager_database_cursor.execute(
                ...
                (username_of_user_to_be_deleted,),
            )
            ...
            if deleted_user:
                ...
                warnings.warn(
                    "called delete user function with a 'None' username, no users were deleted"
                )
    ...
    def login(self, user_to_login):
        if user_to_login:
            ...
            warnings.warn(
                "login function called with a 'none' user, did not log user in"
            )
    ...
    def get_user(self, username):
        if not username:
            warnings.warn(
                "Called get user function with 'None' username, returning None"
            )
            return
    ...
    def get_all_users(self):
        self.usermanager_database_cursor.execute(
            f"SELECT * FROM {self.users_table_name}"
        )
    ...
    def change_user_password(self, username, new_password):
        ...
        if not row:
            warnings.warn(
                f"called change user password function with a none existant username, password not changed"
            )
            return
        ...
        self.usermanager_database_cursor.execute(
            f"UPDATE {self.users_table_name} SET password = ? WHERE username = ?",
    ...
    def change_username(self, old_username, new_username):
        if not old_username:
            warnings.warn(
                "called change useraname function with 'None' old username, did not change username"
            )
            return

        if not new_username:
            warnings.warn(
                "called change username function with 'None' new username, username not changed"
            )
            return
        ...
        if not old_username_exists:
            warnings.warn(
                "called change user name with non existant user, did not change Username"
            )
            return

        if new_username_exists:
            warnings.warn(
                "called change user name with user name that already exist, did not change user name"
            )
            return

        self.usermanager_database_cursor.execute(
            f"UPDATE {self.users_table_name} SET username = ? where username = ?",
    ...
    def user_has_empty_password(self, username):
        if not username:
            warnings.warn(
                "called username has empty password fucntion with none username, returning False"
            )
            return False
        ph = PasswordHasher()
